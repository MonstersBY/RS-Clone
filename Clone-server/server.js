...
const io = new Server(httpServer, {
  cors: {
    origin: [
      ...
      "https://comforting-narwhal-588f6b.netlify.app/",
    ],
  },
});
...
io.on("connection", (socket) => {
  ...
  socket.on("leave-lobby", (room, name) => {
    const index = allrooms.findIndex((findRoom) => findRoom.room === room);
    if (index !== -1) {
      const indexUser = allrooms[index].users.findIndex(
        (findUser) => findUser.username === name
      );
      if (indexUser !== -1) {
    ...
    for (let i = 0; i < allrooms[index].users.length; i++) {
      if (!allrooms[index].users[i].ready) {
        ...
        io.to(room).emit(
          "message",
          "Bot",
          `${allrooms[index].users[i].username} not ready`
        );
      }
    }
  ...
  socket.on("isYouTurnPlayer", (room, name) => {
    const index = allGame
      .get(room)
      .playersInfo.findIndex((findUser) => findUser.name === name);
    const active = allGame.get(room).activePlayer === index ? true : false;
    if (active)
      io.to(room).emit(
        "game-message",
        "Bot",
        `${allGame.get(room).playersInfo[index].name} turn`
      );
    if (allGame.get(room).turn > 0) {
      ...
      socket.emit(
        "firstSettlementMode",
        allGame.get(room).playersInfo[index],
        active
      );
    }
  });
  ...
  socket.on("Next-person", (room) => {
    if (allGame.get(room).turn) {
      if (
        ...
        allGame.get(room).playersInfo.length - 1
      ) {
        allGame.get(room).activePlayer++;
      } else if (
        allGame.get(room).activePlayer >=
  ...
  socket.on("game:victory", (room, player) => {
    io.to(room).emit("game:displayWinner", player);
  });

  // Map events
  socket.on("map:renderAll", (room) => {
    io.to(room).emit("renderFullMapView", allGame.get(room).mapObject);
  });

  socket.on("map:renderHex", (room, hex) => {
  ...
  // Player actions
  // Robb & Roll
  socket.on("player:roll", (room, roll) => {
    allGame.get(room).diceRoll = roll;
    allGame
      .get(room)
      .addResoursesThisTurn(
        roll[0] + roll[1],
        allGame.get(room).mapObject,
        allGame.get(room).playersInfo
      );
  ...
  socket.on("player:setRobber", (room, player, id, knights) => {
    ...
    if (knights) {
      ...
      allGame
        .get(room)
        .calculateMaxArmySize(allGame.get(room), allGame.get(room).playersInfo);
    }
    io.to(room).emit("game:updatePlayerInfo", allGame.get(room).playersInfo);
    io.to(room).emit(
      "renderOneHex",
      allGame.get(room).mapObject[id.split("_")[1]],
      id.split("_")[1]
    );
  ...
  // Cards
  socket.on("player:buyDevCard", (room, player) => {
  ...
  socket.on("player:playMonopolyCard", (room, player, resource) => {
    const index = allGame
      .get(room)
      .playersInfo.findIndex((findUser) => findUser.name === player.name);
    allGame
  ...
  socket.on("player:playPlentyCard", (room, player, resource) => {
    const index = allGame
      .get(room)
      .playersInfo.findIndex((findUser) => findUser.name === player.name);
    allGame
      .get(room)
      .plentyCard(allGame.get(room).playersInfo[index], resource);

    io.to(room).emit("game:updatePlayerInfo", allGame.get(room).playersInfo);
  });

  socket.on("player:playRoadsCard", (room, player) => {
    const index = allGame
      .get(room)
      .playersInfo.findIndex((findUser) => findUser.name === player.name);
